...
# ── ربط الأسطر المكسورة ──
from doc_reader import reflow_wrapped_lines
...
def _reflow_wrapped():
    ...
    out = reflow_wrapped_lines(src)
    assert "وعبدالمجيد إبراهيم بن علي الرزقان وأحمد بن عبدالله بن سليمان" in out
    assert len(out.split("\n")) == 2


check("ربط الأسطر: نصّ مكسور يُدمج في فقرات", _reflow_wrapped)


def _reflow_keeps_clean():
    clean = (
        "المادة الأولى: يُعمل بهذا النظام بعد نشره.\n"
        "المادة الثانية: يُلغى كل ما يخالفه.\n"
        "المادة الثالثة: على الجهات المختصة تنفيذه."
    )
    assert reflow_wrapped_lines(clean) == clean  # منسّق أصلاً → لا يُمسّ


check("ربط الأسطر: منسّق أصلاً لا يُمسّ", _reflow_keeps_clean)
