...
check("تحويل التاريخ: دقّة السنة (إصلاح إزاحة ~سنة) على تواريخ حقيقية", () => {
  // 1426/3/20هـ = أواخر أبريل 2005م (كان الخلل يعطي 2004) — من وثيقة فعلية
  const g1 = convertDateApprox("1426/03/20");
  assert.ok(g1.includes("2005"), `متوقّع 2005، النتيجة: ${g1}`);
  assert.ok(g1.includes("-04-"), `متوقّع أبريل، النتيجة: ${g1}`);
  // 1 محرّم 1426هـ = 2005-02-10م (مرساة معايرة)
  const g2 = convertDateApprox("1426/01/01");
  assert.ok(g2.includes("2005"), `متوقّع 2005، النتيجة: ${g2}`);
  // 1440/01/01هـ = 2018-09-11م تقريباً
  const g3 = convertDateApprox("1440/01/01");
  assert.ok(g3.includes("2018"), `متوقّع 2018، النتيجة: ${g3}`);
});

check("الجودة: صفحات فاشلة تُخفّض التقييم وتمنع «ممتاز» (لا 100% كاذبة)", () => {
  // محاكاة وثيقة 6 صفحات، صفحتان تعذّرت قراءتهما سحابياً (كالوثيقة الحقيقية)
  const body = Array.from({ length: 6 }, (_, i) =>
    `[صفحة ${i + 1}]\n` +
    (i === 2 || i === 4
      ? "(تعذّرت قراءة هذه الصفحة سحابياً)"
      : "حكمت المحكمة برفض الدعوى وألزمت المدّعي بالمصاريف الشرعية وفق الأنظمة.")
  ).join("\n\n");
  const q = assessQuality(body);
  assert.equal(q.failedPages, 2);
  assert.equal(q.grade, "review", "لا يجوز grade=high مع صفحات مفقودة");
  assert.ok(q.score < 85, `يجب أن ينخفض دون 85، كان ${q.score}`);
  assert.ok(/تعذّرت قراءة 2 صفحة/.test(q.label), `التسمية تذكر الصفحات المفقودة: ${q.label}`);
});
...
check("الفساد الدلالي: خربشة جزئيّة بجزرٍ سليمة (تغطية 0.05–0.15) → معطوب + توجيه OCR", () => {
  // النكسة الواقعيّة: طبقةٌ معطوبة تحوي أجزاءً قُرئت صحيحةً («جزرًا سليمة») ترفع
  // التغطية البنيويّة فوق العتبة الصارمة القديمة (0.05) فتفوتها. شظايا بلا معنى
  // + كلمتان وظيفيّتان فقط. (بنية مطابقة لعيّنةٍ حقيقيّة فشلت قبل إعادة المعايرة.)
  const gib =
    "اوهللاد ميد اا ىد بدأم حيدف مخىدا سدعع جىدل ادلئ جدل اات ميلى جوتد جمللىد جوتد وا " +
    "ااهلليد ىددةذم ااعد الاسدل الىلم بمل ملهللي اتف بدلحل اددف كدم بوددله اددس اواهمل ودل ادد " +
    "بلدت تكوم أيد لدو مدهللاا اةجد سذل اول هللي بس يب مدعا حيدف مخىدا";
  const partial = `${gib} على من ${gib}`;
  const r = arabicSemanticCorruptionScore(partial);
  assert.equal(r.corrupt, true);
  assert.ok(
    r.structureCoverage > 0.05 && r.structureCoverage < 0.15,
    "الحالة الجزئيّة: تغطيةٌ منخفضة لكن فوق العتبة الصارمة القديمة"
  );
  assert.equal(cleanPdfTextLayer(partial).needsOcr, true);
});
...
check("ربط الأسطر: نصّ مكسور (حكم مرقّم) يُدمج في فقرات متدفّقة", () => {
  const wrapped =
    "لدينا نحن عبدالله بن عبدالرحمن بن عثمان الأحمد وعبدالمجيد\n" +
    "إبراهيم بن علي الرزقان وأحمد بن\n" +
    "عبدالله بن سليمان العقيلي قضاة هذه الدائرة بموجب القرار الصادر\n" +
    "من فضيلة رئيس المحكمة المساعد بالرقم كذا وتاريخ كذا.\n" +
    "وبناءً على ما تقدّم فقد حكمت الدائرة بما يأتي.";
  const out = reflowWrappedLines(wrapped);
  // الأسطر الثلاثة الأولى (بلا نهاية جملة) تُدمج في سطرٍ واحد متدفّق
  assert.ok(/عبدالمجيد إبراهيم بن علي الرزقان وأحمد بن عبدالله بن سليمان العقيلي/.test(out));
  // الجملة المنتهية بنقطة تبقى فاصلاً
  assert.equal(out.split("\n").length, 2);
});

check("ربط الأسطر: نصّ منسّق أصلاً (فقرات كاملة) لا يُمسّ", () => {
  const clean =
    "المادة الأولى: يُعمل بهذا النظام بعد نشره.\n" +
    "المادة الثانية: يُلغى كل ما يخالفه.\n" +
    "المادة الثالثة: على الجهات المختصة تنفيذه.";
  assert.equal(reflowWrappedLines(clean), clean); // حارس عدم-اللمس
});

check("ربط الأسطر: يحافظ على العناوين والقوائم وعلامات الصفحات", () => {
  const src =
    "[صفحة 1]\n" +
    "هذا نصٌّ طويلٌ يمتدّ على أكثر من سطرٍ دون أن ينتهي\n" +
    "بعلامة جملةٍ بل يستمرّ إلى السطر التالي\n" +
    "ويكمل هنا أيضاً دون انقطاع\n" +
    "حتى ينتهي أخيراً بنقطة.\n" +
    "أولاً: البند الأول من القرار\n" +
    "ثانياً: البند الثاني من القرار";
  const out = reflowWrappedLines(src);
  assert.ok(out.includes("[صفحة 1]"), "علامة الصفحة باقية");
  assert.ok(out.includes("أولاً: البند الأول"), "بداية القائمة لم تُدمج في سابقها");
  assert.ok(out.includes("ثانياً: البند الثاني"));
  assert.ok(/يمتدّ على أكثر من سطرٍ دون أن ينتهي بعلامة جملةٍ بل يستمرّ/.test(out), "أسطر التكملة دُمجت");
});
