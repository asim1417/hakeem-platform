...
assert.ok(home.includes("حوّل الوقائع المتناثرة"));
assert.ok(home.includes("إلى موقف قانوني يمكن البناء عليه"));
assert.ok(home.includes("ليس مجرد جواب. مسار عمل قانوني قابل للمراجعة"));
...
assert.ok(home.includes("ابدأ الآن عبر Google"));
...
assert.ok(page.includes("حوّل الوقائع إلى موقف قانوني مسند"));
...
assert.ok(guestAsk.includes("تعود إليها بعد موافقة Google"));
assert.ok(guestAsk.includes("ابدأ التحليل عبر Google"));
