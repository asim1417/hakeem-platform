...
assert.ok(home.includes("من الوقائع إلى المخرج القانوني"));
assert.ok(home.includes("بحث وتحليل وسند في مسار واحد"));
assert.ok(home.includes("مخرج يمكن مراجعته واستكماله"));
assert.ok(home.includes("منصة تقنية قانونية للممارسة السعودية"));
assert.ok(home.includes("تحرير المسألة"));
assert.ok(home.includes("إسناد التحليل"));
