...
async function main() {
  ...
  assert.ok(sfdaMatch, "SFDA live API did not return the known published CR 7001583587");
  console.log(`PASS | SFDA | observations=${sfdaRaw.observations.length} cr=${sfdaMatch?.commercialRegistration} license=${sfdaMatch?.sourceRecordId ?? "n/a"}`);
  ...
  console.log("Summary: entity-level SFDA and review-only CST probes passed.");
}
