...
/**
 * تحسين خفيف للصورة قبل رفعها لـ Gemini (بديل مكتبة sharp، في المتصفح):
 * تدرّج رمادي + تمديد تباين (normalize) + حدّة يسيرة — يبرز الحبر الباهت دون
 * التحويل الثنائي القاسي الذي يضرّ فهم نموذج الرؤية. يعمل على لوحة مرسومة.
 */
function enhanceCanvasForCloud(canvas: HTMLCanvasElement): HTMLCanvasElement {
  ...
  // تمديد تباين خطّي (normalize)
  for (let i = 0, g = 0; i < px.length; i += 4, g += 1) {
    const n = ((gray[g] - min) * 255) / range;
...
export async function cloudOcrPdfPages(
  ...
  try {
    ...
    const readPage = async (p: number): Promise<string | null> => {
      ...
      enhanceCanvasForCloud(canvas); // تحسين خفيف قبل الرفع (بديل sharp، في المتصفح)
