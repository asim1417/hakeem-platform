 ...
 *
 * الأداء: يفضّل عمود search_norm المفهرس إن وُجد؛ وإلّا يسقط إلى تطبيع فوريّ (ILIKE)
 * صالح للقراءة بلا فهرس (أبطأ). الفهرس النهائي يُطبَّق عبر هجرة على Neon branch لا الإنتاج.
 */
...
export function redactPII(text: string): string {
  if (!text) return "";
  return String(text)
    // هوية/إقامة سعودية: 10 أرقام تبدأ بـ 1 أو 2 (لاتينية أو هندية)
    .replace(/(?<!\d)[12][0-9\u0660-\u0669]{9}(?!\d)/g, "•••••••••• [هوية محجوبة]")
    // جوال سعودي: 05######## أو +9665######## أو 009665########
