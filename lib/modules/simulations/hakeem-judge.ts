...
export function buildJudgeOpening(claim?: ClaimData, title?: string) {
  ...
  return [
    ...
    // إحالةٌ إلى صحيفة الدعوى المقيّدة (فيها الوقائع والطلبات) ثمّ عرضها على المدّعى عليه لطلب الجواب.
    `وبموجب صحيفة الدعوى المقيّدة لدى المنصّة — وموضوعها: ${claim?.subject || title || "غير محدد"} — أحالت الدائرة إليها وأثبتت مضمونها من وقائعَ وطلبات، ثمّ عرضتها على المدّعى عليه وطلبت منه الجواب عنها.`,
    "وتُفهِم الدائرة الطرفين بالتقيّد بالنظام وحسن الأدب في المرافعة، وأنّ القرارات الإجرائيّة تُتّخذ في محالّها قبل الحكم.",
    "والدور الآن للمدّعى عليه للجواب عن صحيفة الدعوى ووقائعها وطلباتها."
  ].join("\n");
}

// نسخُ صحيفة الدعوى بيانًا للمدّعي في المحضر (فتُثبَت دعواه من صحيفته المقيّدة دون إعادة عرضٍ
// شفهيّ)، فيُحال عليها القاضي ويُطلب جواب المدّعى عليه مباشرةً.
export function buildClaimStatement(claim?: ClaimData): string {
  return [
    "بموجب صحيفة الدعوى المقيّدة، تتحصّل دعوى المدّعي فيما يلي:",
    claim?.subject ? `الموضوع: ${claim.subject}` : null,
    claim?.facts ? `الوقائع: ${claim.facts}` : null,
    claim?.requests ? `الطلبات: ${claim.requests}` : null,
    claim?.legalGrounds ? `الأساس النظاميّ المذكور: ${claim.legalGrounds}` : null
  ]
    .filter(Boolean)
    .join("\n");
}
...
// مسودة اعتراضٍ حتميّة (سقوطٌ آمنٌ حين يتعذّر الحكم المؤصَّل): بمصطلحات كلّ مسارٍ وبنيةٍ قضائيّة
// موجزة، دون منطوقٍ صوريّ مُدَّعًى — بيانُ حالٍ لا حكمٌ مختلق.
export function buildAppealDraft(kind: string, reasons: string[]) {
  const isNaqd = /نقض/.test(kind);
  const isIltimas = /التماس/.test(kind);
  const party = isNaqd ? "طالب النقض" : isIltimas ? "الملتمِس" : "المستأنِف";
  const court = isNaqd ? "المحكمة العليا" : isIltimas ? "محكمة الاستئناف المؤيِّدة" : "دائرة الاستئناف";
  return [
    `مسودة ${kind} — بيئة محاكاة تدريبيّة`,
    "بسم الله الرحمن الرحيم",
    `الجهة المختصّة: ${court} — الطرف المُعترِض: ${party}.`,
    "أولًا: قبول الاعتراض شكلًا",
    "يُشترط تقديم الاعتراض خلال الميعاد النظاميّ مستوفيًا شروط مذكّرته، وإلا رُدّ شكلًا.",
    "ثانيًا: أسباب الاعتراض",
    reasons.length ? reasons.map((r, i) => `${i + 1}. ${r}`).join("\n") : "لم تُحدَّد أسبابٌ مبنيَنة بعد.",
    "ثالثًا: الطلب",
    `يلتمس ${party} من ${court} النظر في أسباب اعتراضه أعلاه وفق أحكام لائحة الاعتراض على الأحكام.`,
    "تنبيه: هذه مسودةٌ تدريبيّة غير ملزمة، ولا تُعدّ إجراءً قضائيًّا فعليًّا. (تعذّر توليد الحكم المؤصَّل — يتطلّب تفعيل القاضي الذكيّ وتوفّر أساسٍ نظاميّ.)"
  ].join("\n\n");
}
