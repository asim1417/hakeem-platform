...
test("isKnown يقبل السوابق الشائعة", () => {
  ...
  // «ال» لا تجتمع مع ضمير: «الخكم» لا تُقبل على أنها «الخ» + «كم».
  addText(l, "إلخ", 50);
  assert.ok(!isKnown(l, "الخكم"));
});
...
test("خلط ى/ي في آخر الكلمة", () => {
  const l = lex();
  const m = measureRuling(l, "حكمت المحكمة في الدعوي");
  assert.equal(m.spelling.yaAlif, 1); // «الدعوي» بدل «الدعوى»
});
