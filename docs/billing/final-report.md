...
| `npm run test:billing-ratewindow` | نافذة تحديد المعدّل | **7/7 ✅** |

**الإجمالي: 108/108 اختبار وحدة ناجح.** `tsc --noEmit` نظيف (خروج 0) عبر المشروع كامله بعد كل مرحلة.
...
## 3. اكتمل لاحقًا («نفّذ كل شيء»)

1. **تعميم الخصم على كل خدمات الذكاء** ✅ — حارس موحّد `service-guard.ts` مربوط بالمسارات السبعة غير المقيسة (اسأل/اسأل-stream/legal-chat/تحليل/خطط/تقدير/صياغة): 402 عند نقص الرصيد، 429 عند تجاوز الحد، تثبيت عند النجاح، تحرير عند الفشل، والـstreaming يُثبّت عند نقطة الإنهاء. no-op حتى تفعيل العلمين.
2. **Rate limiting على مسارات AI** ✅ — نافذة ثابتة عبر `generic_rate_limit_windows` (`rate-window.ts`، 7 اختبارات).
3. **تشفير provider tokens** ✅ — `payment-token.ts` (AES-256-GCM عبر `settings-service`)؛ الحفظ عبر `POST /api/billing/payment-methods` مشفّرًا، والتجديد يفكّ التشفير قبل الخصم.
4. **مقاعد OFFICE/ENTERPRISE** ✅ — محفظة فريق مشتركة (`resolveWalletOwner`)، إدارة الأعضاء ضمن حدّ الباقة (`/api/billing/workspace/*`)، و`getWalletSummary` يعرض رصيد المساحة للأعضاء.
5. **لوحة الربحية والإدارة** ✅ — `/admin/billing/{plans,service-rates,profitability,payments,users}` + خدمة `profitability.ts` (تكلفة Claude/الهامش لكل خدمة).
6. **ربط التجربة** ✅ — `POST /api/billing/trial/claim` (60+40، مُجزّأ بالهوية، خلف علم النظام) دون لمس المسارات الساخنة.

### يبقى بطبيعته خارج نطاق الكود
- **ZATCA**: الواجهة والحقول والعلم جاهزة، لكن **يتطلب ربط مزود متوافق واجتياز الاختبارات النظامية/الشهادة** — لا يُدّعى التوافق قبلها.
- **اختبارات التكامل/E2E الحيّة**: مكتوبة كمخرجات، تحتاج staging + Moyasar Sandbox للتشغيل.
