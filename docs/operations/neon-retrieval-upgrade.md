...
سرّ **`DATABASE_URL` في GitHub يجب أن يشير إلى Neon (كتابة)** — لا إلى قاعدة الأتمتة الأخرى (Supabase). وإلا ستُطبَّق الترقية على القاعدة الخطأ. هذا هو سبب القفل: مصدر الحقيقة وقت التشغيل هو Neon، بينما سرّ الأتمتة الحالي يشير إلى قاعدة أخرى.

**خطوتك:** حدّث سرّ `DATABASE_URL` (أو أضف اتصال Neon) في إعدادات المستودع، ثم شغّل الـworkflow بالتأكيد.
