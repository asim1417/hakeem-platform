...
بأمر المالك (الدمج والنشر دون توقّف) اعتُمد النموذج الحي بدل بناء `legal_work`:

- العمل الجديد → `LegalSystem` (ديباجة أداة الإصدار على النظام، لا مادة صفر).
- المادة/البند → `LegalArticle` مع `search_norm` ووسم `source:uqn`.
- الرابط والبصمة وتاريخ الجلب → `legal_text_provenance` (جدول إضافي، خارج Prisma حتى لا ينكسر التطبيق قبل الهجرة).
- الإحلال المؤكَّد بالاسم والعدد → إغلاق النسخة المفتوحة ثم `ArticleVersion` جديدة. النص السابق لا يُحذف.
- النظام الذي حلّ محله نظام باسم مختلف → تبقى مواده وتُعلَّم `ملغاة`.
- `parse_status != complete` والآثار `pending_review` لا تُرحَّل إلى النص الساري.

## هـ) ما كان يوجب التوقّف (§8) قبل أمر النشر
