...
export function CopyLinkButton({ label = "نسخ رابط البحث" }: { label?: string }) {
  ...
  return (
    <button
      ...
      onClick={copy}
      aria-label={label}
      className="inline-flex items-center gap-1 rounded-full border border-[var(--ink-20)] bg-ivory px-2.5 py-0.5 text-xs font-semibold text-[var(--navy)] transition hover:border-[var(--gold)]"
    >
