  ...
  return (
    <div dir="rtl">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {matrix.map((row) => (
          <article key={row.role} className="rounded-lg border border-gray-200 bg-white p-4">
              ...
              {row.cells.map((cell) => {
                ...
                return (
                  <li key={cell.permission} className="flex items-center gap-2 border-t border-gray-100 py-1.5 text-sm">
                    <span className={cell.effective ? "text-gray-800" : "text-gray-400"}>{cell.label}</span>
                    {cell.locked ? (
                      <span className="ms-auto rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-500">أساسية 🔒</span>
                    ) : (
                      <button
                        type="button"
                        disabled={busy === id}
