...
export async function DueDiligenceHistory() {
  ...
  const latestEntities = [...grouped.values()].slice(0, 8).map((group) => {
    ...
    const computed = currentMeta ? compareDueDiligenceSnapshots(currentMeta, previousMeta) : null;
    return {
      current,
      currentMeta,
      change: currentMeta?.changes ?? computed,
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 id="dd-history-title" className="mt-1 text-xl font-bold text-slate-950">آخر حالات العناية الواجبة</h2>
            <p className="mt-1 max-w-3xl text-sm text-slate-500">
              يقارن حكيم كل فحص حي بالفحص السابق للكيان. تعطل المصدر لا يُعامل كاختفاء للواقعة، بل يظهر كتغيّر تشغيلي مستقل.
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {latestEntities.map(({ current, currentMeta: meta, change, previousAt }) => {
            ...
            const unavailable = change?.unavailableSources.length ?? 0;
            ...
            return (
              <article key={current.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="rounded-xl bg-white px-3 py-2 text-center shadow-sm">
                    <div className="flex items-baseline justify-center gap-1">
                      ...
                      {riskDelta ? (
                        <span className={`text-[10px] font-bold ${(change?.riskDelta ?? 0) > 0 ? "text-rose-700" : "text-emerald-700"}`}>
                          {riskDelta}
                        </span>
                      ) : null}
                    </div>
                ...
                <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-600">
                  ...
                  {typeof successful === "number" && typeof configured === "number" ? <span>المصادر: {successful}/{configured}</span> : null}
                  {typeof meta?.coverage?.verifiedEvidence === "number" ? <span>أدلة متحققة: {meta.coverage.verifiedEvidence}</span> : null}
                </div>
                    ...
                    {change.baseline ? (
                      ...
                      <div className="space-y-2">
                        <div className="flex flex-wrap gap-2 text-[11px] font-semibold">
                          {newFindings > 0 ? <span className="rounded-full bg-amber-100 px-2.5 py-1 text-amber-800">نتائج جديدة: {newFindings}</span> : null}
                          {noLongerObserved > 0 ? <span className="rounded-full bg-slate-200 px-2.5 py-1 text-slate-700">لم تعد ظاهرة: {noLongerObserved}</span> : null}
                          {unavailable > 0 ? <span className="rounded-full bg-sky-100 px-2.5 py-1 text-sky-800">مصادر غير متاحة: {unavailable}</span> : null}
                          {change.sourceStatusChanges.length > 0 ? <span className="rounded-full bg-violet-100 px-2.5 py-1 text-violet-800">تغير حالة المصادر: {change.sourceStatusChanges.length}</span> : null}
                        </div>
                        <p className="text-[11px] leading-5 text-slate-500">
                          مقارنة بالفحص السابق: {formatDate(change.comparedToGeneratedAt ?? previousAt)}. «لم تعد ظاهرة» لا تعني زوال الواقعة، وأي نتيجة أخفاها تعطل مصدر لا تُحسب كإزالة.
                        </p>
                      </div>
                    ) : (
