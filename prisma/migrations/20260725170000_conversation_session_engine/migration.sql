-- محرك الجلسات الموحّد — توسيع chat_conversations / chat_messages + ربط generation_jobs
-- ترتيب داخلي: أعمدة nullable → backfill → قيود (بلا DEFAULT دائم على service_key)

-- ── 0) جداول قد تُنشأ خارج migrate (idempotent) قبل FKs ─────────────────────
...
-- ── 3) قيود (بلا DEFAULT على service_key) ───────────────────────────────────
